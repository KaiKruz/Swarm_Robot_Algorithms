...
NUM_ROBOTS = 50  # Increased number of robots
MAX_VELOCITY = 3  # Increased max velocity
INITIAL_INERTIA = 0.9  # Initial inertia weight
MIN_INERTIA = 0.4  # Minimum inertia weight
...
RANDOM_EXPLORATION_THRESHOLD = 10  # Reduced threshold for random exploration
EARLY_STOPPING_THRESHOLD = 100  # Increased early stopping threshold
ENERGY_CONSUMPTION_RATE = 0.05  # Reduced energy consumption rate
INITIAL_ENERGY = 150.0  # Increased initial energy for each robot
...
def simulate_pso(robots):
    ...
    def update_plot(iteration):
        ...
        # Visualization
        ax.clear()
        # Create a coverage map for visualization
    ...
    # Recompute the final coverage_map
    coverage_map = np.zeros((GRID_SIZE, GRID_SIZE))
    for robot in robots:
        for cell in robot.covered_area:
            coverage_map[cell] = 1

    # Proceed with plotting KPIs
    ...
    fig_plotly.update_layout(
        title=f'Final Coverage: {best_coverage:.2%}',
        xaxis_title='X-axis',
        yaxis_title='Y-axis',
...
# Initialize robots with random starting positions within the grid
robots = [Robot(
    start_position=(
